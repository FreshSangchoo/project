...
interface SellerTransactionProps {
  userId: number;
}

function SellerTransaction({ userId }: SellerTransactionProps) {
  const insets = useSafeAreaInsets();
  const navigation = useExploreNavigation();
  const rootNavigation = useRootNavigation();
  const [merchandiseData, setMerchandiseData] = useState([]);
  const [sort, setSort] = useState<SortValue>('latest');
  const { getUserProductList } = useMyPostsApi();
  ...
  const fetchPosts = async (nextPage: number, reset = false, sortParam?: SortValue) => {
    ...
    try {
      const params = { page: nextPage, size: 10, sort: sortParam ?? sort };
      const res = await getUserProductList(userId, params);
        ...
        ListEmptyComponent={
          initialLoading ? (
            ...
            <NoResultSection
              ...
              title="아직 거래한 악기가 없어요."
              description="첫 거래를 만들어보세요!"
            />
          )
        }
      />
