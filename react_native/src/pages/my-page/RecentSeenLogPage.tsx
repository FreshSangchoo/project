...
function RecentSeenLogPage() {
  const navigation = useMyNavigation();
  const { getRecentList } = useMyPostsApi();
  ...
  const load = async (targetPage: number, mode: 'reset' | 'append') => {
    ...
    try {
      const { posts, totalCount, currentPage, pageCount } = await getRecentList({ page: targetPage, size, sort });
  ...
  return (
    <SafeAreaView style={styles.recentSeenLogPage} edges={['top', 'right', 'left']}>
      <CenterHeader
        title="최근 본 악기"
        leftChilds={{
          icon: (
...
export default RecentSeenLogPage;
