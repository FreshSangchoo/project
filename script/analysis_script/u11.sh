...
function U-11() {
    ...
    for account in "${SYSTEM_ACCOUNTS[@]}"; do
        if grep -q "^${account}:" /etc/passwd 2>/dev/null; then
            ...
            if [[ ! "$ACCOUNT_SHELL" =~ (nologin|false|sync|shutdown|halt)$ ]]; then
                ...
                DETAILS_ARRAY+=("{\"점검항목\":\"시스템 계정 Shell\",\"상태\":\"취약\",\"세부내용\":\"취약: ${account} 계정의 쉘이 ${ACCOUNT_SHELL}로 설정되어 있음\"}")
            fi
        fi
    done
    ...
    if [ ${#VULNERABLE_ACCOUNTS[@]} -eq 0 ]; then
        ...
        DETAILS_ARRAY+=("{\"점검항목\":\"시스템 계정 Shell\",\"상태\":\"양호\",\"세부내용\":\"양호: 로그인이 필요하지 않은 계정에 안전한 쉘 설정됨\"}")
    else
        STATUS="VULNERABLE"
