...
function U-22() {
    ...
    if [ ! -f "$TARGET_FILE" ]; then
        ...
        DETAILS_ARRAY+=("{\"점검항목\":\"$TARGET_FILE\",\"상태\":\"양호\",\"세부내용\":\"양호: $TARGET_FILE 파일이 존재하지 않아 해당 취약점 없음. ()\"}")
    else
        ...
        if [[ "$FILE_OWNER" == "root" || "$FILE_OWNER" == "bin" || "$FILE_OWNER" == "sys" ]]; then
            ...
            IS_VULN=1
            DETAILS_ARRAY+=("{\"점검항목\":\"소유자 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: 소유자가 $FILE_OWNER 입니다. (root, bin, sys 중 하나여야 함)\"}")
        fi

        # 3. 권한 확인 (644 이하)
        if [ "$FILE_PERM" -le 644 ]; then
            DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"양호\",\"세부내용\":\"양호: 권한이 $FILE_PERM 입니다. (기준: 644 이하)\"}")
        else
            IS_VULN=1
            DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: 권한이 $FILE_PERM 입니다. (기준: 644 이하)\"}")
        fi
