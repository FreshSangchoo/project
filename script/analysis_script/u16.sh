...
function U-16() {
    ...
    if [ "$FILE_OWNER" == "root" ]; then
        DETAILS_ARRAY+=("{\"점검항목\":\"소유자 설정\",\"상태\":\"양호\",\"세부내용\":\"양호: 소유자가 root 입니다.\"}")
    else
        IS_VULN=1
    ...
    if [ "$FILE_PERM" -le 644 ]; then
        DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"양호\",\"세부내용\":\"양호: 권한이 $FILE_PERM 입니다. (644 이하)\"}")
    else
        IS_VULN=1
        DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: 권한이 $FILE_PERM 입니다. (644 이하 설정 필요)\"}")
    fi
