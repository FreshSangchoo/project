...
function U-63() {
    ...
    if [ ! -f "$TARGET_FILE" ]; then
        DETAILS_ARRAY+=("{\"점검항목\":\"$TARGET_FILE\",\"상태\":\"양호\",\"세부내용\":\"양호: $TARGET_FILE 파일이 존재하지 않습니다. (sudo 패키지 미설치 추정)\"}")
    else
        ...
        if [ "$FILE_OWNER" != "root" ]; then
            IS_VULN=1
            DETAILS_ARRAY+=("{\"점검항목\":\"소유자 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: 소유자가 root가 아닙니다.\"}")
            #DETAILS_ARRAY+=("\"   >> 조치: chown root $TARGET_FILE\"")
        else
            DETAILS_ARRAY+=("{\"점검항목\":\"소유자 설정\",\"상태\":\"양호\",\"세부내용\":\"양호: 소유자가 root입니다.\"}")
        fi
        ...
        if [ "$FILE_PERM" -gt 640 ]; then
            IS_VULN=1
            DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: 권한이 640을 초과합니다.\"}")
            #DETAILS_ARRAY+=("\"   >> 조치: chmod 640 $TARGET_FILE (또는 440)\"")
        else
            DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"양호\",\"세부내용\":\"양호: 권한이 640 이하입니다.\"}")
        fi
