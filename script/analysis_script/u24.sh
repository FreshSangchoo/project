           ...
           [ -d "$home_dir" ]; then
            ...
            for env_file in "${ENV_FILES[@]}"; do
                ...
                if [ $is_file_vuln -eq 1 ]; then
                    ((VULNERABLE_COUNT++))
                    IS_VULN=1
                    DETAILS_ARRAY+=("{\"점검항목\":\"$file_path\",\"상태\":\"취약\",\"세부내용\":\"취약: $file_path - $vuln_reason\"}")
                else
                    ((SAFE_COUNT++))
                    DETAILS_ARRAY+=("{\"점검항목\":\"$file_path\",\"상태\":\"양호\",\"세부내용\":\"양호: $file_path (소유자: $file_owner, 권한: $file_perm)\"}")
                fi
            done
        fi
    done < /etc/passwd
