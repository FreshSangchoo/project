...
function U-18() {
    ...
    if [ ! -f "$TARGET_FILE" ]; then
        ...
        DETAILS_ARRAY+=("{\"점검항목\":\"$TARGET_FILE\",\"상태\":\"취약\",\"세부내용\":\"취약(수동점검): $TARGET_FILE 파일이 존재하지 않습니다.\"}")
    ...
    if [ "$FILE_OWNER" == "root" ]; then
        ...
        IS_VULN=1
        DETAILS_ARRAY+=("{\"점검항목\":\"소유자 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: 소유자가 $FILE_OWNER 입니다. (기준 : root여야 함)\"}")
    fi

    # 3. 권한 확인 (400 이하)
    # 400(r--------) 이하인 경우(000 포함) 양호로 판단
    if [ "$FILE_PERM" -le 400 ]; then
        DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"양호\",\"세부내용\":\"양호: 권한이 $FILE_PERM 입니다. (기준: 400 이하)\"}")
    else
        IS_VULN=1
        DETAILS_ARRAY+=("{\"점검항목\":\"권한 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: 권한이 $FILE_PERM 입니다. (기준: 400 이하)\"}")
    fi
