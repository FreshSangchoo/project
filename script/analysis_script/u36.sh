...
function U-36() {
    ...
    if command -v systemctl >/dev/null 2>&1; then
        for svc in $SERVICE_LIST; do
            if systemctl is-active --quiet "$svc" 2>/dev/null; then
                IS_VULN=1
                DETAILS_ARRAY+=("{\"점검항목\":\"Systemd\",\"상태\":\"취약\",\"세부내용\":\"취약: Systemd 서비스 $svc 가 가동 중입니다.\"}")
            fi
        done
    fi
        ...
        for r_svc in rlogin rsh rexec shell login exec; do
            if [ -f "/etc/xinetd.d/$r_svc" ]; then
                if grep -vE "^#|^\s*#" "/etc/xinetd.d/$r_svc" | grep -q "disable[[:space:]]*=[[:space:]]*no"; then
                    IS_VULN=1
                    DETAILS_ARRAY+=("{\"점검항목\":\"xinetd 설정\",\"상태\":\"취약\",\"세부내용\":\"취약: xinetd 내 $r_svc 서비스가 활성화되어 있습니다.\"}")
                fi
            fi
        done
    fi
    ...
    if [ -f /etc/inetd.conf ]; then
        if grep -vE "^#|^\s*#" /etc/inetd.conf | grep -qE "rlogin|rsh|rexec|shell|login|exec"; then
            IS_VULN=1
            DETAILS_ARRAY+=("{\"점검항목\":\"etc/inetd.conf\",\"상태\":\"취약\",\"세부내용\":\"취약: /etc/inetd.conf 내 r 계열 서비스 설정이 존재합니다.\"}")
        fi
    fi
    ...
    if [ -f "/etc/hosts.equiv" ]; then
        IS_VULN=1
        DETAILS_ARRAY+=("{\"점검항목\":\"etc/hosts.equiv\",\"상태\":\"취약\",\"세부내용\":\"취약: /etc/hosts.equiv 파일이 존재합니다 (무인증 접속 위험)\"}")
    fi
    local root_rhosts="/root/.rhosts"
    if [ -f "$root_rhosts" ]; then
        IS_VULN=1
        DETAILS_ARRAY+=("{\"점검항목\":\"$root_rhosts\",\"상태\":\"취약\",\"세부내용\":\"취약: $root_rhosts 파일이 존재합니다 (무인증 접속 위험)\"}")
    fi
    ...
    if [ $IS_VULN -eq 1 ]; then
        ...
        DETAILS_ARRAY+=("{\"점검항목\":\"불필요한\",\"상태\":\"양호\",\"세부내용\":\"양호: 불필요한 r 계열 서비스가 비활성화되어 있습니다.\"}")
