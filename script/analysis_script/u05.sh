...
function U-05() {
    ...
    if [ -z "$UID_ZERO_ACCOUNTS" ]; then
        ...
        DETAILS_ARRAY+=("{\"점검항목\":\"UID 0 계정 확인\",\"상태\":\"취약\",\"세부내용\":\"취약: UID 0인 계정을 찾을 수 없음\"}")

    elif [ "$UID_ZERO_ACCOUNTS" = "root" ]; then
        STATUS="SAFE"
        CURRENT_VALUE="root"
        DETAILS_ARRAY+=("{\"점검항목\":\"UID 0 계정 확인\",\"상태\":\"양호\",\"세부내용\":\"양호: root 계정만 UID 0을 가지고 있음\"}")
        ...
        while IFS= read -r account; do
            if [ -n "$account" ] && [ "$account" != "root" ]; then
                DETAILS_ARRAY+=("{\"점검항목\":\"UID 0 계정 확인\",\"상태\":\"취약\",\"세부내용\":\"취약: ${account} 계정이 UID 0을 사용 중\"}")
            fi
