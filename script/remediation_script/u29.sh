#!/bin/bash
###############################################################################
# [U-29] /etc/hosts.lpd 파일 소유자 및 권한 설정 - 개별 조치
# 점검: 소유자 root, 권한 600 이하 / 조치: chown root, chmod 600
###############################################################################
...
remediate_u29() {
    ...
    local DESCRIPTION="/etc/hosts.lpd 파일 소유자 및 권한 조치"
    local ACTIONS_TAKEN=()
    local DETAILS=()
    local DETAIL_OBJS=()
    local STATUS="SAFE"
    ...
    local P_NUM P_OWN; P_NUM=$(stat -c "%a" "$TARGET_FILE" 2>/dev/null); P_OWN=$(stat -c "%U" "$TARGET_FILE" 2>/dev/null)
    [ -n "$P_NUM" ] && [ -n "$P_OWN" ] && BEFORE="${BEFORE} (권한: ${P_NUM}, 소유자: ${P_OWN})"

    if [ "$P_OWN" != "root" ] || { [ -n "$P_NUM" ] && [ "$P_NUM" -gt 600 ]; }; then
        mkdir -p /tmp/security_audit/backup/U-29
        cp -p "$TARGET_FILE" "/tmp/security_audit/backup/U-29/hosts.lpd.bak_$(date +%Y%m%d_%H%M%S)" 2>/dev/null
        chown root "$TARGET_FILE" 2>/dev/null
        chmod 600 "$TARGET_FILE" 2>/dev/null
        REMEDY_CMD="chown root $TARGET_FILE; chmod 600 $TARGET_FILE"
        ACTIONS_TAKEN+=("소유자/권한 변경: $P_OWN/$P_NUM → root/600")
    fi

    AFTER=$(ls -l "$TARGET_FILE" 2>/dev/null)
    local P_AFTER_NUM P_AFTER_OWN; P_AFTER_NUM=$(stat -c "%a" "$TARGET_FILE" 2>/dev/null); P_AFTER_OWN=$(stat -c "%U" "$TARGET_FILE" 2>/dev/null)
    [ -n "$P_AFTER_NUM" ] && [ -n "$P_AFTER_OWN" ] && AFTER="${AFTER} (권한: ${P_AFTER_NUM}, 소유자: ${P_AFTER_OWN})"

    if [ -n "$REMEDY_CMD" ]; then
        if [ "$P_AFTER_OWN" != "root" ] || { [ -n "$P_AFTER_NUM" ] && [ "$P_AFTER_NUM" -gt 600 ]; }; then
            STATUS="VULNERABLE"
            ...
            DETAILS+=("조치 완료: /etc/hosts.lpd 소유자 및 권한 조치")
        fi
    else
        DETAIL_OBJS+=("$(build_detail_obj_full "양호" "$BEFORE" " " "$AFTER" "기존 양호여서 조치 없음" "$CRITERIA")")
        DETAILS+=("양호: /etc/hosts.lpd 소유자 및 권한 기준 충족")
    fi

    for act in "${ACTIONS_TAKEN[@]}"; do DETAILS+=("조치완료: $act"); done
    local PRE_VAL="소유자 $P_OWN, 권한 $P_NUM"; local POST_VAL="소유자 $P_AFTER_OWN, 권한 $P_AFTER_NUM"
